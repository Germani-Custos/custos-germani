...
## MNT-01 · 🟠 Médio · `view/ui-controller.js` é um "god module" (1.144 linhas) — ✅ Resolvido

- **Local:** `view/ui-controller.js` inteiro.
- **Resolução (2026-07-27):** fatiamento concluído; `view/ui-controller.js` atua predominantemente como orquestrador e a fila investigativa foi extraída para `view/ui-table.js`.
...
- **Progresso (em andamento):**
  ...
  - [x] **Fila/tabela e presenter investigativo** (2026-07-27) — `view/ui-table.js` (`createTableController({ dom, executeOperationalBoundary, renderDrillThrough, rerunReportForProduct })`) concentra `renderTable`, `getOperationalPriority`, `buildInvestigativeSummary`, `formatCurrencyCell` e `formatDiffCell`. `ui-controller.js` injeta apenas a fronteira ERR-01, o drill-through e o re-run silencioso do relatório, preservando comportamento. Esta fatia encerra o MNT-01; a discussão futura do MNT-08 fica reduzida à decisão de remover ou manter a injeção dessas funções em `ui-export.js`.
...
- **Local:** `getOperationalPriority` e `buildInvestigativeSummary` em `view/ui-table.js`. São funções puras (sem DOM) de classificação/redação investigativa, mas vivem no orquestrador de bootstrap/renderização. Usadas pela tabela (`renderTable`) **e** pela exportação (`view/ui-export.js`), que as recebe por **injeção** (`createExportController({ ..., getOperationalPriority, buildInvestigativeSummary })`).
